...
### **Автор исследования**
**LintixerDA**, а также некоторые мысли **expeexpeexpe** (Александр) и часть результатов от **guidiki**
...
Таким образом, были изменены параметры ОС. Также в результатах будет участвовать только утилита wrk. Во время данной работы было обнаружно, что hey значительно слабее нагружает сервер по сравнению с wrk, хотя нагрузка на тестеры оставалась одинаковой. Стоит еще учесть тот момент, что обе эти утилиты крайне неравномерно нагружают все ядра процессора. И еще у них ограниченный функционал по сравнению с другими, например, JMeter или K6. Таким образом, выбор этих утилит был не самым удачным.  
Следует еще уточнить, что на результаты тестов мог влиять хостинг. Каким образом? При моем тестировании количество запросов в секунду было до 72к единиц. В то же время у другого человека (@guidiki), который сам арендовал себе сервера для теста, количество запросов в секунду при меньших использований ресурсов tester достигали до 667349 (и это, то, что он скинул). Таким образом, хостинг может ограничивать что-либо, например, количество подключений. Также не исключено, влияние предварительных настроек ОС у хостингов. Результаты команды на одном tester показаны ниже:  
```
[secret@ip ~]$ wrk -t18 -c50000 -d2m -H 'User-Agent: TestAgent' http://ip:8081/                                                       
Running 2m test @ http://ip:8081/
  18 threads and 50000 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency    65.35ms  122.52ms   1.19s    88.13%
    Req/Sec    41.30k    23.14k  120.98k    65.86%
  80152246 requests in 2.00m, 63.67GB read
  Socket errors: connect 24766, read 0, write 0, timeout 0
Requests/sec: 667349.24
Transfer/sec:    542.88MB
```
Вернемся к основному тесту. На рисунке 1 показан скрин нагрузки на CPU на tester через мониторинг. Смотреть преимущественно стоит только с промежутка 23:20 до 23:50. Как видно, нагрузка не достигла даже 50%. При попытке увеличить нагрузку, tester начинал работать нестабильно.
  
![image](https://github.com/user-attachments/assets/a1a3d9f8-b739-4679-96d9-c92332a8ae38)
<p align="center">
Рисунок 1 - Нагрузка CPU на одном из Tester
</p>  
  
При этом во всех тестах нагрузка на CPU на целевом сервере была до 2100% (по мониторингу контейнеров), но по графику, который изображен на рисунке 2, CPU был нагружен по итогу всего на 40%. Результаты лучше также смотреть с 23:20 по 23:50.
  
![image](https://github.com/user-attachments/assets/fc24b67e-a27c-46ed-9843-617439eb2ae7)
<p align="center">
Рисунок 2 - Нагрузка CPU на целевом сервере
</p>  
...
На рисунках 3-11 представлены результаты выполнения команды, а также нагрузка на целевой сервер.  
С учетом того, что ресурсы на целевом сервере не были использованы даже на половину, будем считать, что сервер не испытывал никаких проблем при нагрузках. Таким образом в данном случае важны результаты принятых запросов, а также ошибочных запросов.
```  
wrk -t32 -c40000 -d1m -H "User-Agent: CustomAgent" http://ip/index.html
```  
...
<p align="center">
Рисунок 9 - Результаты Tester-1 (Подсистема хоста)
</p>  
  
![image](https://github.com/user-attachments/assets/64e215e9-f86f-4227-bba2-74f5fe5a7088)
<p align="center">
Рисунок 10 - Результаты Tester-2 (Подсистема хоста)
</p>  
  
![image](https://github.com/user-attachments/assets/f6edad3f-c85f-4e93-b72f-09d746b5cc4d)
<p align="center">
Рисунок 11 - Нагрузка на сервер (Подсистема хоста)
</p>  
  
Также на рисунке 12 представлен график успешных и неуспешных запросов:
  
![image](https://github.com/user-attachments/assets/b9c0c4b8-ce6e-44ff-90c6-6df8087d9f96)
<p align="center">
Рисунок 12 - График запросов с двух tester
</p>  
  
Как видно, в двух из трех тестов Tester 1 по какой-то причине не захотел адекватно отправлять запросы. В рамках ограниченного времени не удалось это заметить сразу, поэтому отбросим первый тестер и будем сравнивать только по второму. В данном случае, это допустимо, так как целевой сервер не испытывал никаких проблем при нагрузках.  
Также оба тестера отправили наименьшее количество запросов на хост. Связано это из-за сервера? У меня сомнения, так как по логике данный способ должен быть лучше. Опустим общее количество запросов для данного теста и посмотрим на процент ошибочных запросов к общему количеству (но для интереса все равно укажу общее количество).  
Новый график изображен на рисунке 13:  
  
![image](https://github.com/user-attachments/assets/d94006bd-c0e5-4538-959f-000d633263db)
<p align="center">
Рисунок 13 - График запросов со второго tester
</p>  
...
<p align="center">
Рисунок 14 - Итоговый график
</p>  
  
График на рисунке 14 иллюстрирует, что конфигурация с использованием сетевой подсистемы хоста через Docker обеспечивает наилучший баланс между количеством обработанных запросов и минимальным числом ошибок. В то время как Nginx, работающий на хосте (Port 6969), демонстрирует наибольшее число ошибок, что указывает на наихудшую производительность в данном тестировании.
...
Тест выявил множество факторов, которые нужно учитывать в нагруженных системах, начиная от тюнинга тестеров до конфигурации веб-сервера. Также для таких исследований необходимо выделить достаточное количество времени.
